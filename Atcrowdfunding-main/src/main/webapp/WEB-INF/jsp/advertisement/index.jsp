...
<body>
	...
	<script type="text/javascript">
            ...
            function changePageno( currentPage ) {
            	//window.location.href = "<%=request.getContextPath() %>/advertisement/index.htm?currentPage="+currentPage;
            ...
            function queryPage( pageIndex ){
            	...
            	var loadingIndex = -1 ;
            	$.ajax({
            		url : "${APP_PATH}/advertisement/list.do",
            		type : "post",
            		data : dataObj,
            		beforeSend : function(){
            		...
            		success : function(ajaxResult){
            		    //console.log(ajaxResult.success)
            			//显示结果
            			layer.close(loadingIndex);
            			if(ajaxResult.success==true){
            				//循环遍历,显示数据
            				var pageObj = ajaxResult.page;
            				var list = pageObj.datas ;
            				/*console.log("数据：",pageObj,list);*/
            				var content = "";
            				$("tbody").html(content);
            				$.each(list,function(i,n){
								...
								content+="	<td>";
								content+="		<button type='button' onclick='window.location.href=\"${APP_PATH}/advertisement/toUpdate.htm?currentPage="+pageObj.currentPage+"&id="+n.id+"\"' class='btn btn-primary btn-xs'>";
            ...
            function deleteAdvert(id,name){
    			...
    			layer.confirm("是否要删除广告?",  {icon: 3, title:'提示'}, function(cindex){
    				...
    				$.ajax({
    					url : "${APP_PATH}/advertisement/toDelete.do",
    					type : "POST",
    					data : {id:id},
    					success : function(ajaxResult){
    						if(ajaxResult.success==true){
            ...
            $("#batchDelete").click(function(){
            	...
            	if(checkedList.length <= 0){
                    layer.msg("至少选中一个广告",{time:100,icon:5,shift:6});
                    return;
            	}
                layer.confirm("确定要删除吗?",  {icon: 3, title:'提示'}, function(cindex){
                    ...
                    $.each(checkedList,function(i,n){
                        ...
                        datas["advertisementList["+i+"].id"]=n.value;
                        datas["advertisementList["+i+"].name"]="n.name";
                    });

                    $.ajax({
                        url : "${APP_PATH}/advertisement/toBatchDeleteAdvertisement.do",
                        type : "POST",
                        data : datas ,
                        success : function(ajaxResult){
                            if(ajaxResult.success==true){
