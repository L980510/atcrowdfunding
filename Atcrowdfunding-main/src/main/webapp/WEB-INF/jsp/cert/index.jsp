                ...
                if (ajaxResult.success == true) {
                   ...
                   // alert(datas)
                    ...
                    $.each(datas, function (i, n) {
                        ...
                        content += '  <td>';
                        content += '<button type="button" onclick=\'window.location.href="${APP_PATH}/cert/assignPermission.htm?certid='+n.id+'"\' class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
                        // content+='	  <button type="button" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
                        content += "		<button type='button' onclick='window.location.href=\"${APP_PATH}/cert/update.htm?id=" + n.id + "\"' class='btn btn-primary btn-xs'>",
    ...
    $("#BatchToDeleteBtn").click(function () {
       ...
       // alert(selectcheckbox.length);
        ...
        layer.confirm("你确定要删除该资质吗？，能给个机会吧！",{icon:3,title:"删除提示"},function (cindex) {
            $.ajax({
                type:"POST",
                url:"${APP_PATH}/cert/toBatchDeleteRole.do",
                data:dataObj,
                beforeSend:function () {
                    //校验表单数据
                    loadingIndex=layer.load("正在删除数据",{time:1000,icon:6});
                ...
                success:function (ajaxResult) {
                    ...
                    if (ajaxResult.success) {
                        layer.msg("删除成功！",{time:1000,icon:5,shift:6});
                        //删除成功跳转到资质tolist页面
                        window.location.href="${APP_PATH}/cert/tolist.htm";
                        //queryPageUser(1);
                    }else{
                        layer.msg(ajaxResult.message,{time:1000,icon:5,shift:6});
                    }
