        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form class="form-inline" param="form" style="float:left;">
                    ...
                    <button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APP_PATH}/param/add.htm'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
                    <br>
                    <hr style="clear:both;">
...
<script type="text/javascript">
                ...
                if (ajaxResult.success == true) {
                   ...
                   // alert(datas)
                    ...
                    $.each(datas, function (i, n) {
                        ...
                        content += '  <td>' + n.val + '</td>';
                        content += '  <td>';
                        content += '<button type="button" onclick=\'window.location.href="${APP_PATH}/param/assignPermission.htm?paramid='+n.id+'"\' class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
                        // content+='	  <button type="button" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
                        content += "		<button type='button' onclick='window.location.href=\"${APP_PATH}/param/update.htm?id=" + n.id + "\"' class='btn btn-primary btn-xs'>",
    ...
    function deleteUser(id,name) {
        var loadingIndex=-1;
        layer.confirm("你确定要删除["+name+"]该资质？",{icon:3,title:"删除提示"},function (cindex) {
            $.ajax({
                type:"POST",
                url:"${APP_PATH}/param/toDelete.do",
                data:{
                    id:id
                },
                beforeSend:function () {
    ...
    $("#BatchToDeleteBtn").click(function () {
        ...
        if (selectcheckbox.length<=0) {
            layer.msg("至少选中一个资质",{time:100,icon:5,shift:6});
            return;
        }
       // alert(selectcheckbox.length);
        ...
        var dataObj={};
        $.each(selectcheckbox,function (i,n) {
            dataObj["paramList["+i+"].id"]=n.id;
            console.log(n.name,n.id);
            dataObj["paramList["+i+"].name"]=n.name;
        })
        layer.confirm("你确定要删除该资质吗？，能给个机会吧！",{icon:3,title:"删除提示"},function (cindex) {
            $.ajax({
                type:"POST",
                url:"${APP_PATH}/param/toBatchDeleteRole.do",
                data:dataObj,
                beforeSend:function () {
                    //校验表单数据
                    loadingIndex=layer.load("正在删除数据",{time:1000,icon:6});
                ...
                success:function (ajaxResult) {
                    ...
                    if (ajaxResult.success) {
                        layer.msg("删除成功！",{time:1000,icon:5,shift:6});
                        //删除成功跳转到资质tolist页面
                        window.location.href="${APP_PATH}/param/tolist.htm";
                        //queryPageUser(1);
                    }else{
                        layer.msg(ajaxResult.message,{time:1000,icon:5,shift:6});
                    }
