        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form class="form-inline" type="form" style="float:left;">
                    ...
                    <button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APP_PATH}/type/add.htm'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
                    <br>
                    <hr style="clear:both;">
...
<script type="text/javascript">
                ...
                if (ajaxResult.success == true) {
                   ...
                   // alert(datas)
                    ...
                    $.each(datas, function (i, n) {
                        ...
                        content += '  <td>';
                        content += '<button type="button" onclick=\'window.location.href="${APP_PATH}/type/assignPermission.htm?typeid='+n.id+'"\' class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
                        // content+='	  <button type="button" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
                        content += "		<button type='button' onclick='window.location.href=\"${APP_PATH}/type/update.htm?id=" + n.id + "\"' class='btn btn-primary btn-xs'>",
    ...
    function deleteUser(id,name) {
        var loadingIndex=-1;
        layer.confirm("你确定要删除["+name+"]该分类？",{icon:3,title:"删除提示"},function (cindex) {
            $.ajax({
                type:"POST",
                url:"${APP_PATH}/type/toDelete.do",
                data:{
                    id:id
                },
                beforeSend:function () {
    ...
    $("#BatchToDeleteBtn").click(function () {
        ...
        if (selectcheckbox.length<=0) {
            layer.msg("至少选中一个分类",{time:100,icon:5,shift:6});
            return;
        }
       // alert(selectcheckbox.length);
        ...
        var dataObj={};
        $.each(selectcheckbox,function (i,n) {
            dataObj["typeList["+i+"].id"]=n.id;
            console.log(n.name,n.id);
            dataObj["typeList["+i+"].name"]=n.name;
        })
        layer.confirm("你确定要删除该分类吗？，能给个机会吧！",{icon:3,title:"删除提示"},function (cindex) {
            $.ajax({
                type:"POST",
                url:"${APP_PATH}/type/toBatchDeleteRole.do",
                data:dataObj,
                beforeSend:function () {
                    //校验表单数据
                    loadingIndex=layer.load("正在删除数据",{time:1000,icon:6});
